...
namespace nhlbi_toolbox
{
    namespace corrections
    {
        mri_concomitant_field_correction::mri_concomitant_field_correction(const ISMRMRD::IsmrmrdHeader &head)
        {
            ...
            if( head.acquisitionSystemInformation.get().systemFieldStrength_T > 0)
                B0 = 1.5;
            else
        ...
        bool mri_concomitant_field_correction::is_setup()
        {
            return isSetup;
        }
...
#pragma omp parallel for
        ...
        void mri_concomitant_field_correction::setup(hoNDArray<floatd3> gradients, ISMRMRD::AcquisitionHeader acq_header)
        {
            ...
            demodulation_freqs = arma::linspace(freqMap.min()-0.2*freqMap.min(), freqMap.max() * 1.2, numfreqbins);
